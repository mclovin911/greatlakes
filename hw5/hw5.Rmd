...
# Data Preprocessing
load("~/greatlakes/hw5/mnist.RData")
x_train <- x_train/255
x_test <- x_test/255
...
# SVM
library(e1071)
t =tune(svm, lab~., data=train_data, ranges=list(cost=c(10,100, 300, 500), kernel=c("polynomial", "radial")), tunecontrol = tune.control(cross=5), scale=FALSE)
svm_model <- t$best.model
saveRDS(t$best.model, 'svm_model')
svm_model <- readRDS('svm_model')
```
